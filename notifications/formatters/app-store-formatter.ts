...
export class AppStoreFormatter extends BaseFormatter {
  ...
  private formatAppMessage(notification: NotificationPayload): SlackMessage {
    ...
    const emoji = notification.status.getAppStoreStatusEmoji()

    const message = `${emoji} The status of your app *${appName}* has been changed to *${status}*`
  ...
  private formatBuildMessage(notification: NotificationPayload): SlackMessage {
    ...
    const emoji = status.getAppStoreStatusEmoji()

    const message = `${emoji} The status of build version *${version}* for your app *${appName}* has been changed to *${status}*`
