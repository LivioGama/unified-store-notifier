...
export class PlayStoreFormatter extends BaseFormatter {
  async formatMessage(notification: NotificationPayload): Promise<SlackMessage> {
    ...
    if (!lastTrackInfo || lastTrackInfo.status === 'EMPTY') {
      message = `New release detected for *${packageName}* with version code *${currentVersionCode}*`
    } else if (currentVersionCode !== lastVersionCode) {
      message = `Version code updated for *${packageName}*: *${lastVersionCode}* → *${currentVersionCode}*`
    } else {
      const status = notification.status
        .replace(/_/g, ' ')
        .toLowerCase()
        .replace(/\b\w/g, l => l.toUpperCase())
      message = `The status of your app *${packageName}* has been changed to *${status}*`
    }
