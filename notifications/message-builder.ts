...
export class MessageBuilder {
  ...
  private buildFallbackMessage(notification: NotificationPayload): SlackMessage {
    return {
      text: `📱 ${notification.appName} - ${notification.status}`,
      attachments: [
        {
  ...
  buildTestMessage(): SlackMessage {
    return {
      text: '🧪 Store Notifier Test Message',
      attachments: [
        {
          color: 'good',
          title: 'Connection Test Successful',
          text: 'The unified Store Notifier is working correctly and can send messages to Slack.',
          fields: [
            {
              title: 'Supported Platforms',
              value: 'App Store Connect, Google Play Console',
              short: true,
            },
            {
              title: 'Status',
          ...
          footer: 'Store Notifier • System Test',
          ts: Math.floor(Date.now() / 1000).toString(),
        },
      ],
    }
