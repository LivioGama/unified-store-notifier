...
export class AppStoreProcessor implements PlatformProcessor {
  ...
  private analyzeBuild = async (
    app: AppStoreApp,
    build: AppStoreBuild,
  ): Promise<AppStoreNotificationData> => {
    const buildKey = `${app.bundleId}-${build.version}-${build.buildNumber}`
    const storedBuild = database.get<AppStoreBuild>('app-store', buildKey)

    const isNewBuild = !storedBuild && dayjs(build.uploadDate).isAfter(dayjs().subtract(30, 'day'))
    const statusChange = storedBuild?.status !== build.status
    ...
    return {
      ...
      isNewBuild,
      buildComparison: {
        versionChanged: storedBuild?.version !== build.version,
        buildNumberChanged: storedBuild?.buildNumber !== build.buildNumber,
